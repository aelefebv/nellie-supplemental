...
def run_mitograph(frame_dir, lateral_pixel_size, axial_pixel_size, mitograph_dir):
    ...
    print(command)
    try:
        subprocess.run(command, shell=True, timeout=300, check=True)  # 300 seconds or 5 minutes, 7200 seconds for multi-grid
...
def parallel_process_files(top_dirs):
    ...
    with Pool(num_processes) as pool:
        ...
        for result in results:
            try:
                result.get(timeout=360)  # 6 minutes to give a bit extra time over run_mitograph, 7260 seconds for multi-grid
            except:
