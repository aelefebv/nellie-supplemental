...
if __name__ == "__main__":
    ...
    bit_depth_max = 65535
    intensity = bit_depth_max // 2  # keep it in the middle so no risk of over or under saturation
    ...
    for px_size_um in px_size_vector:
        ...
        for thickness in thickness_vector_px:
            for length in length_vector_px:
                ...
                sub_volume = create_skeleton_sub_volume(length=length_actual)
                ...
                print(f'px_size_um: {px_size_um:.2f}, length_actual: {length_actual:.2f}, thickness_actual: {thickness_actual:.2f}')
                save_name_no_ext = f'px_size_{px_size_um:.2f}-length_{length_actual:.2f}-thickness_{thickness_actual:.2f}'
                ...
                for std in [128, 256, 512, 1024, 2048, 4096, 8192]:
                    noisy_im_series = []
                    for t in range(3):
                        noisy_im_series.append(add_noise(new_im, psf_sigma=sigma_psf, gaussian_mean=0, gaussian_std=std,
                                                         apply_poisson=True))
