...
def get_nellie_outputs(csv, tif, time_bw_frames, px_size_um):
    ...
    # grab voxel movement, relabelled branch label displacements, relabelled object label displacements
    # new df with only 'lin_vel_mag_raw' and 't' columns
    nellie_df_small = nellie_df[['t', 'reassigned_label_raw', 'label']]
    # add 5 to the reassigned labels
...
def get_stats(displacement_dict):
    ...
    # middle frame is a transition and we don't have exact displacement values for it, so remove.
...
raw_path = "/Users/austin/GitHub/nellie-simulations/motion/linear/outputs"
save_path = os.path.join(raw_path, "raw_outputs.csv")
...
for raw_basename in raw_basenames_no_ext:
    ...
    long_axis = str(raw_basename.split('-')[1].split('_')[-1])
    ...
    tnt_displacement, tnt_persistance = get_mitotnt_seg_outputs(mitotnt_csv, time_bw_frames, False)
